...
assert(/if\(\(w\.model\|\|''\)!==\(url\|\|''\)\)\{ w\.view = null; w\.clips = undefined; _gunClipNames\[key\] = \[\]; \}/.test(src), 'changing the weapon URL clears the old view + animation memory (clips + discovered clip names) so the new model re-frames and re-resolves');
