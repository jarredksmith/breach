...
assert(/if\(!names\.size && store\.animClipNames\)\{ for\(const nm of store\.animClipNames\)\{ if\(nm\) names\.add\(nm\); \} \}/.test(src), 'falls back to the trigger prop\'s own clips');
...
assert(/s\.target=ti\.value\.trim\(\); rerender\(\);/.test(src), 'editing the target tag refreshes the clip dropdown (shared editor)');
