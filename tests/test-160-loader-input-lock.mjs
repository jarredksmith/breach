...
assert(/if\(!_mantle && _jPressed && player\.onGround && \(player\.jumpCd\|\|0\)<=0 && !_levelLoaderActive && matchWarmup<=0 && !mountedTurret\)\{ player\.vel\.y = JUMP/.test(src), 'jump blocked while loading (and during the pre-match warmup)');
